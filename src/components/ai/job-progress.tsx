...
import type { JobStatus } from "@/components/ai/job-status-badge";
...
export function JobProgress({ jobId, initialStatus, initialProgress }: JobProgressProps) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await fetch(`/api/ai/jobs/${jobId}/status`, {
          cache: "no-store",
        });
        if (!response.ok) {
          setGaveUp(true);
        ...
        const data = (await response.json()) as { status: JobStatus; progress: number };
